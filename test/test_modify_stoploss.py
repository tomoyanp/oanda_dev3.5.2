...
def calc_stoploss_metric(instrument, insert_time, price, trade_side):
    length_list = [21, 50, 75, 100, 200]
    table_type = "1m"
    insert_time = insert_time - timedelta(minutes=1)

    for length in length_list:
        sma = get_sma(instrument, insert_time, table_type, length)
    ...
    return metric_length, table_type
...
def decide_trade(insert_time, length_list):
    ...
    if trade_flag and flag:
        ...
        if sma_flag:
            ...
            sma_length, sma_tabletype = calc_stoploss_metric(instrument, insert_time, price, trade_side)
            trade_obj["flag"] = True
            ...
            trade_obj["stoploss_metric_tabletype"] = sma_tabletype
            trade_obj["stoploss_metric_length"] = sma_length
        else:
            pass
