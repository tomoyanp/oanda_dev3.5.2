...
def candle_stick_1h(price_df):
    ...
    fig = plt.figure(figsize=(10, 5))
    ax = fig.add_subplot(1, 1, 1)
    mpl_finance.candlestick_ohlc(ax, data, width=0.5/(24), alpha=0.5, colorup="r", colordown="b")

    ax.grid()

    locator = mdates.HourLocator(interval=5, tz=None)
    ax.xaxis.set_major_locator(locator)
    ax.xaxis.set_major_formatter(mdates.DateFormatter("%m/%d\n%H:%M"))
