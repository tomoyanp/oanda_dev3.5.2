...
while insert_time < now:
    ...
    for instrument in instrument_list:
        sql = "select open_ask, open_bid, close_ask, close_bid, insert_time from %s_1h_TABLE where insert_time < '%s' order by insert_time desc limit 1" % (instrument, insert_time)
    ...
    if result_list[0] < 1.0 and result_list[1] < 1.0 and result_list[2] < 1.0 and result_list[5] < 1.0:
        print("%s: EUR_JPY sell" % insert_time)
    if result_list[0] < 1.0 and result_list[1] < 1.0 and result_list[2] < 1.0 and result_list[5] > 1.0:
        print("%s: EUR_USD sell" % insert_time)
    if result_list[0] > 1.0 and result_list[3] < 1.0 and result_list[4] < 1.0 and result_list[5] < 1.0:
        print("%s: GBP_JPY sell" % insert_time)
    if result_list[0] > 1.0 and result_list[3] < 1.0 and result_list[4] < 1.0 and result_list[5] > 1.0:
        print("%s: GBP_USD sell" % insert_time)
