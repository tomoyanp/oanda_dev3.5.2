...
def decide_trade(trade_flags, insert_time):
    ...
    # 週末の3時以降ならやめる
        ...
        trend_ema = ema[ema["insert_time"] <= tmp["insert_time"][trend_length-1]]
        trend_ema = trend_ema.tail(trend_length).reset_index(drop=True)
        trend_count = pd.DataFrame()
        trend_count["buy"] = (tmp["close"] > trend_ema["ema25"])
        trend_count["buy"] = len(trend_count["buy"][trend_count["buy"] == True])
        trend_count["sell"] = (tmp["close"] < trend_ema["ema25"])
        ...
        if trade_flags["direction"] != "flat" and trade_flags["touched_ema"]:
            ...
            current_ema_df = current_ema_df.reset_index(drop=True)
            # EMA25を0.1以上割り込んだらリセットする
            if trade_flags["direction"] == "buy" and current_ema_df["ema25"][0] - current_price > 0.1:
                trade_flags = reset_trade_flags()
            elif trade_flags["direction"] == "sell" and current_price - current_ema_df["ema25"][0] > 0.1:
                trade_flags = reset_trade_flags()
